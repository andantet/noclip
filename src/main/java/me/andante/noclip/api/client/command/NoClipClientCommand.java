...
public interface NoClipClientCommand {
    ...
    SimpleCommandExceptionType CONFIG_SYNTAX_EXCEPTION = new SimpleCommandExceptionType(new TranslatableText("text." + NoClip.MOD_ID + ".config_reload_syntax_error"));
    SimpleCommandExceptionType TOGGLE_EXCEPTION = new SimpleCommandExceptionType(new TranslatableText("text." + NoClip.MOD_ID + ".not_toggle_error"));
    ...
    static int executeConfigReload(CommandContext<FabricClientCommandSource> context) throws CommandSyntaxException {
        ...
        context.getSource().sendFeedback(new TranslatableText(CONFIG_RELOAD_KEY).setStyle(NoClipClient.getTextStyle()));
        return 1;
    }
}
